...
class PETR3D(DETR):
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        Args:
            img (list[torch.Tensor]): List of multiple images.  # (bs, 3, 1241, 800)
        ...
        feat = self.extract_feat(img)  # 特征图, backbone + neck
    ...
    def show_result(self,
        ...
        if isinstance(result, tuple):
            bbox_result, keypoint_result = result  # [100, 5], [100, 17, 3]
            segm_result = None
        else:
            bbox_result, segm_result, keypoint_result = result, None, None
        bboxes = np.vstack(bbox_result)  # [100, 5]
        labels = [
    ...
    def imshow_det_bboxes(self,
        ...
        num_keypoint = keypoints.shape[1]  # 17
        if num_keypoint == 14:
